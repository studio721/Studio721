...
function validateYouTubeUrl(url) {
    ...
    if (match && match[2].length == 11) {
        ...
        chrome.tabs.query({}, tabs => {
            tabs.forEach(tab => {
            chrome.tabs.sendMessage(tab.id, {sendMessage:true, data:`studio721,youtube,${url}`});
          });
        });

        // sendMessage({sendMessage:true, data:`studio721,youtube,${url}`})
...
$('#play-pause').on('click', function () {
    ...
    if (x.innerHTML === "pause") {
        ...
        chrome.tabs.query({}, tabs => {
            tabs.forEach(tab => {
            chrome.tabs.sendMessage(tab.id, {sendMessage:true, data:`studio721,play`});
          });
        });
        player.pauseVideo();
        ...
        chrome.tabs.query({}, tabs => {
            tabs.forEach(tab => {
            chrome.tabs.sendMessage(tab.id, {sendMessage:true, data:`studio721,pause`});
          });
        });
        player.playVideo();
